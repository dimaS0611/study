...
public class Index implements Serializable, Closeable {
    ...
    IndexOne2One            tariff;
    IndexOne2One          name;
IndexOne2One          reduction;
IndexOne2One          date;
IndexOne2One          startTalk;
IndexOne2One          endTalk;

    public void test( Conversation conv ) throws KeyNotUniqueException {
        assert( conv != null );
        if ( name.contains(conv.name)) {
            throw new KeyNotUniqueException( conv.name );
        }
        if ( number.contains( conv.number )) {
            throw new KeyNotUniqueException( conv.number );
        }
    }

    public void put( Conversation conv, long value ) throws KeyNotUniqueException {
        test( conv );
        name.put( conv.name, value );
        number.put( conv.number, value);
        tariff.put( conv.tariff, value);
    }

    public Index()  {
        name   = new IndexOne2One();
        number = new IndexOne2One();
        tariff 	= new IndexOne2One();
    }
